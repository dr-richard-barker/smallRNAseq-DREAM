...
PAIRED="${PAIRED:-auto}"           # yes | no | auto (detect _R1_/_R2_ pairs)
...
: > counts.long.tsv
# detect paired-end (R1/R2) unless overridden; paired reads are aligned as pairs to avoid
# double-counting the same fragment against a precursor.
have_pairs=no
ls reads/*_R1*.f*q.gz >/dev/null 2>&1 && have_pairs=yes
if { [ "$PAIRED" = yes ] || { [ "$PAIRED" = auto ] && [ "$have_pairs" = yes ]; }; }; then
  echo "   paired-end mode"
  for r1 in reads/*_R1*.f*q.gz; do
    [ -e "$r1" ] || continue
    r2="${r1/_R1/_R2}"
    s=$(basename "$r1" | sed 's/_R1.*//')
    bowtie2 --local --no-unal --no-mixed --no-discordant -p 4 -x "${SPECIES}_hairpin" \
      -1 "$r1" -2 "$r2" 2>"${s}.bt2.log" | samtools sort -o "${s}.bam" - && samtools index "${s}.bam"
    # count read PAIRS per precursor (one fragment = one count): properly-paired, primary, first-in-pair
    samtools view -c -f 66 -F 256 "${s}.bam" >/dev/null 2>&1 || true
    samtools idxstats "${s}.bam" | awk -v s="$s" '$1!="*" && $3>0{print $1"\t"s"\t"int($3/2)}' >> counts.long.tsv
  done
else
